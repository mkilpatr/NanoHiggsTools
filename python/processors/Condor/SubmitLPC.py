...
def SplitPro(key, file, lineperfile=10, eventsplit=2**19, TreeName=None):
    if args.region != "": eventsplit=2**21
...
def my_process(args):
    ...
    global tempdir
    global ProjectName
    ProjectName = time.strftime('%b%d') + ShortProjectName + VersionNumber + args.process + args.region + "_again"
    if args.era == 0:
